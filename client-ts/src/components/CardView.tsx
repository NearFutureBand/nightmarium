...
import { AbilityState, Card, LegionState, Monster, Player } from '../types';
import { MONSTER_PART } from '../img';
import { ABILITIES, ABILITY_TYPE, BODYPARTS, COLORS } from '../constants';
import { useAppDispatch, useAppSelector } from '../app/hooks';
import { selectAvailableBodyPartsToInstall, selectIsActive, setDraggedCard, setSelectedCard } from '../slices/App';
...
const hasMatchingPlaceToBeInstalled = (card: Card, availableBodyparts: Set<number>): boolean => {
  return card.bodypart.some((bodypartIndex) => availableBodyparts.has(bodypartIndex));
};
...
export const CardWithImage: FC<Props> = ({
  ...
  const handleClick: React.MouseEventHandler<HTMLDivElement> = useCallback(
    ...
    [card.bodypart, card.id, card.legion, clickable, dispatch, monster?.id, multiSelectAllowed, player?.id]
  );
  ...
  return (
    ...
    >
      {!image && (
        <>
          <div> id: {card.id}</div>
          <div>{card.bodypart.map((bodypartIndex) => BODYPARTS[bodypartIndex]).join(' | ')}</div>
