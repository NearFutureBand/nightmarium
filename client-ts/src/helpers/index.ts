...
import { AbilityState, Legion, SelectedCard, SelectedCardShort, SelectedMonster } from '../types';

export const getCardIndexInSelection = (cardData: SelectedCardShort, selectedCards: SelectedCardShort[]): number => {
  return selectedCards.findIndex((card) => {
    return card.cardId === cardData.cardId && card.monsterId === cardData.monsterId && card.playerId === cardData.playerId;
  });
};

export const isCardInSelection = (cardData: SelectedCardShort, selectedCards: SelectedCardShort[]) => {
  return getCardIndexInSelection(cardData, selectedCards) >= 0;
};

export const defineCardSelectionMode = (isShiftKeyPressed: boolean, isCardAlreadySelected: boolean) => {
...
export const validateCardToMonster = ({
  ...
  if (!selectedCard.cardBodypart.some((bodypartIndex) => bodypartIndex === selectedMonster.monsterBodyLength)) {
    return `Карта ${selectedCard.cardBodypart.map((bp) => BODYPARTS[bp]).join('/')} не может быть установлена в слот для ${
...
const backgrounds = ['#ffcc8e', 'rgb(226, 153, 153)', '#b6e7b9', '#8fc3ce', '#87bee3', '#c1b3f7', '#cfa6e1', '#f590b6'];
